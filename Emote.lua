    ...
    for _, child in pairs(frontFrame:GetChildren()) do
        if child:IsA("ImageLabel") then
            if buttonIndex <= #currentPageAnimations then
                ...
                local idValue = child:FindFirstChild("AnimationID") or Instance.new("IntanimationData.id
...
local function createGUIElements()
    ...
    -- UIListLayout.Parent = Under -- Dihapus karena _1left dan _9right dipindahkan
    -- UIListLayout.FillDirection = Enum.FillDirection.Horizontal
    -- UIListLayout.VerticalAlignment = Enum.VerticalAlignment.Center

    -- [[ MODIFIKASI UNTUK _1LEFT ]]
    _1left.Name = "1left"
    _1left.Parent = emotesWheel -- **Ubah Parent ke emotesWheel**
    ...
    _1left.Position = UDim2.new(-0.10, 0, 0.5, 0) -- **Atur posisi di kiri (0.02, tengah vertikal 0.5)**
    _1left.AnchorPoint = Vector2.new(0.5, 0.5) -- **Setel AnchorPoint ke tengah**
    _1left.Rotation = 0 -- Hapus rotasi
    _1left.Size = UDim2.new(0.2, 0, 0.2, 0) -- **Sesuaikan Ukuran agar lebih jelas**
    _1left.Image = "rbxassetid://93111945058621"
    _1left.ImageColor3 = Color3.fromRGB(0, 0, 0)
    _1left.ImageTransparency = 0.400

    -- [[ MODIFIKASI UNTUK _9RIGHT ]]
    _9right.Name = "9right"
    _9right.Parent = emotesWheel -- **Ubah Parent ke emotesWheel**
    ...
    _9right.Position = UDim2.new(1.10, 0, 0.5, 0) -- **Atur posisi di kanan (0.98, tengah vertikal 0.5)**
    _9right.AnchorPoint = Vector2.new(0.5, 0.5) -- **Setel AnchorPoint ke tengah**
    _9right.Rotation = 0 -- Hapus rotasi
    _9right.Size = UDim2.new(0.2, 0, 0.2, 0) -- **Sesuaikan Ukuran agar lebih jelas**
...
local function onCharacterAdded(character)
 ...
 if getgenv().autoReloadEnabled and getgenv().lastPlayedAnimation then
    ...
    getgenv().Notify({
        Title = 'ZorVex | Auto Reload Animation',
...
local function checkAndRecreateGUI()
    ...
    if not emotesWheel:FindFirstChild("Under") or not emotesWheel:FindFirstChild("Top") or
        ...
        not emotesWheel:FindFirstChild("Changepage") or not emotesWheel:FindFirstChild("Reload") or
